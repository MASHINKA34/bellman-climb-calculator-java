...
public class BellmanSolver {
    ...
    private ClimbSolution compute(boolean recursive) {
        ...
        //#массивы - две двумерные таблицы размером "уровни на скорости".
        //           valueFunction хранит числа (сколько стоит), policy - индексы (что выбрали)
        //#память - O(N*M): ничего не копим по ходу, только эти две таблицы
        //
    ...
    private double[] buildAltitudeLevels() {
        ...
        //#вещественные - почему нельзя сравнивать double в лоб
        //
    ...
    //#сложность - O(N*M^2) по времени, O(N*M) по памяти.
    //             N - число этапов, M - число скоростей. полный перебор был бы O(M^N)
    //
        ...
        for (int k = altitudes.length - 2; k >= 0; k--) {     // этапы, с предпоследнего вниз
            for (int i = 0; i < speeds.length; i++) {         // откуда стартуем
                ...
                for (int j = 0; j < speeds.length; j++) {     // куда переходим = управление
                    ...
                    //#рекуррентность - вот она: f_k считается через готовое f_k+1.
                    // это и есть уравнение Беллмана, развёрнутое в цикл
    ...
    //#физика - вся математика задачи собрана здесь, в одном методе
    //#формулы - Hэ = H + V²/2g,  dHэ/dt = V(P−Q)/mg,  dt по правилу трапеций
    //
    ...
    //#восстановление - как из таблицы policy получается сама траектория
    //#коллекции - результат копится в ArrayList<TrajectoryPoint>
    //
