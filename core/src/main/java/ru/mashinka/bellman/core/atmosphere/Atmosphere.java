...
//#статика - класс-утилита: все методы static, объект создавать не нужно
//#константы - физические постоянные вынесены в static final, а не размазаны по коду
//#гост - модель атмосферы взята не с потолка, а из ГОСТ 4401-81
//
// стандартная атмосфера по ГОСТ 4401-81, до 20 км
// до 11 км температура падает с градиентом 0,0065 К/м, выше - изотермический слой 216,65 К
//
// класс-утилита: final и с приватным конструктором, чтобы его нельзя было
// ни унаследовать, ни создать. состояния у него нет, объект был бы бессмыслицей
